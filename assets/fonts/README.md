...
这些字体用于《千字文》各讲“逐字精讲”的隶书、楷书、行书、草书展示，避免依赖证书失效的第三方图片域名。

楷书、行书、草书的 WOFF2 文件是从下列授权 TTF 原始字体生成的站内字集；新增讲次前必须把新字加入字集并检查 CMap 覆盖，不能让浏览器回退到系统字体。

当前 WOFF2 子集覆盖第 1–15 讲逐字精讲的 120 个展示字；每次扩展讲次时须同步重建并核验此覆盖范围。
...
- `ZhiMangXing-Regular.woff2`：智莽行书，SIL Open Font License 1.1；完整许可证见 `licenses/ZhiMangXing-OFL.txt`。
- `LiuJianMaoCao-Regular.woff2`：刘建毛草，SIL Open Font License 1.1；完整许可证见 `licenses/LiuJianMaoCao-OFL.txt`。
